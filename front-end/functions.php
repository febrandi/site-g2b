...
if(isset($_POST['login'])){
    ...
    if($usuario != "" && $senha != ""){
        if(strlen($usuario) > 5){
            ...
            setcookie('idioma', 'brazuca', time() + 3600);

            header('Location:index.php');

        }else{
            echo "Usuario deve ser maior que 5";
        }
    }else{
        echo "Campos login e senha não devem ser vazios";
    }
}
...
if (isset($_POST['deletar-produto'])) {
    ...
    $arrayProdutos = json_decode($jsonProdutos, true);
    $pos = $_POST['pos'];
    unlink($arrayProdutos['produtos'][$pos]['imagem']);
